...
export const movementAssessmentCopy = {
  ...
  exploreTipsLabel:
    'Explore tips on managing movement',
...
export const movementExerciseUrl =
  'https://muscha.org/exercise';

export function getMovementImpactPhrase(
  score: number,
) {
  if (score <= 5) {
...
export function getMovementAssessmentPeriodLabel(
  today = new Date(),
) {
  const periodEnd =
    new Date(today);
