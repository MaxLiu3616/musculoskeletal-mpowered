...
export const managementPainTipsUrl =
  'https://muscha.org/pain-guide/';

export function getManagementPeriodLabel(
  today = new Date(),
) {
  const periodEnd =
    new Date(today);
...
export function findExerciseFrequencyLabel(
  id: ExerciseFrequencyId,
) {
  return exerciseFrequencyOptions.find(
    (option) =>
      option.id === id,
  )?.label;
}
...
export function getMedicationResult(
  ...
  if (prescribedNames.length > 0) {
    results.push(
      `You recorded ${prescribedNames.join(
        ', ',
      )}.`,
    );
  }
  ...
  if (otcMedication) {
    results.push(
      `You also recorded ${otcMedication} as an OTC medication.`,
    );
  }
...
export function getExerciseResult(
  ...
  return `You exercised for ${findExerciseFrequencyLabel(
    id,
  )} this week.`;
}

export function getEmotionResult(
  strategy: string,
) {
  const trimmedStrategy =
    strategy.trim();
...
export function getManagementOverview(
  ...
  if (
    responses.medications.length > 0 ||
    responses.otcMedication.trim()
  ) {
    actions.push(
  ...
  if (
    ...
    actions.push(
      'exercised to manage your pain',
    );
  }

  if (
    responses.emotionStrategy.trim()
  ) {
    actions.push(
      'used an emotional wellbeing strategy',
    );
  }
