...
export default function MyHealthScreen({ onProfile, onTracking, onPrescriptions, onHome, onCarePlanner }: MyHealthScreenProps) {
  ...
  return (
    <HealthScreen title={myHealthCopy.title} footer={
      <BottomNavigation activeItem="my-health" onItemPress={(id) => {
        if (id === 'pain-tracker') onHome();
        if (id === 'care-planner') onCarePlanner();
      }} />
    }>
