...
export function painChartSvg(records: AssessmentRecord[], metric: PainMetric) {
  ...
  const grid = Array.from({ length: 11 }, (_, value) => {
    ...
    return `<line x1="28" x2="${width - 12}" y1="${y}" y2="${y}" stroke="${colors.border}" stroke-dasharray="2 3"/><text x="20" y="${y + 4}" text-anchor="end" font-size="10" fill="${colors.muted}">${value}</text>`;
  }).join('');
  const area = points.length > 1 ? `<polygon points="${points[0].x},224 ${path} ${points.at(-1)!.x},224" fill="${colors.sky}"/>` : '';
  const labels = points.map((point) => `<circle cx="${point.x}" cy="${point.y}" r="3.5" fill="${colors.primary}"/><text x="${point.x}" y="${point.y - 9}" text-anchor="middle" font-size="11" fill="${colors.ink}">${point.value}</text><text x="${point.x}" y="245" text-anchor="middle" font-size="10" fill="${colors.muted}">${point.date}</text>`).join('');
  return { width, xml: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} 260" width="${width}" height="260">${area}${grid}<polyline points="${path}" fill="none" stroke="${colors.primary}" stroke-width="2"/>${labels}</svg>` };
}
...
export function healthReportHtml(title: string, sections: HealthSection[], chart = '') {
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/><title>${escapeHtml(title)}</title><style>
    ...
    body { font: 14px/1.5 Arial, sans-serif; color: ${colors.ink}; margin: 24px auto; max-width: 760px; padding: 0 16px; }
    h1 { font-size: 25px; } h2 { font-size: 18px; margin: 0 0 16px; }
    section { background: ${colors.canvas}; border-radius: 12px; padding: 18px; margin: 18px 0; }
    .item { break-inside: avoid; margin: 12px 0; overflow-wrap: anywhere; } p { margin: 3px 0 10px; white-space: pre-wrap; }
    svg { width: 100%; height: auto; } .note { font-size: 11px; color: ${colors.muted}; }
    button { padding: 12px 20px; border: 0; border-radius: 20px; background: ${colors.primary}; color: white; cursor: pointer; }
